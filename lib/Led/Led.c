...
esp_err_t set_led_high()
{
  ...
  if(esp_error == ESP_OK) { led_status = 1; }
  return esp_error;
}
...
esp_err_t set_led_low()
{
  ...
  if(esp_error == ESP_OK) { led_status = 0; }
  return esp_error;
}
...
esp_err_t set_led_toggle()
{
  ...
  if(esp_error == ESP_OK) { led_status = (led_status + 1) & 1; }
  return esp_error;
}
...
void led_task(void* pvParameter)
{
  if(_water_sensor_data.xSemaphore != NULL) {
    ...
    for(;;) {
      ...
      // If the water sensor value is greater than the threshold, turn on the
      // led
